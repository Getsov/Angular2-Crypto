  ...
  loadPost() {
    this.postService.getPostById(this.postId).subscribe(data => {
      if (data['success']) {
        ...
        if(data['post']['likes'].indexOf(this.currentUserId) === -1) {
          this.likeBtn = "Like";
        } else {
          this.likeBtn = "Unlike";
        }
    ...
    if (this.currentUserId) {
      this.postService.likeAndUnlike(payload).subscribe(data => {
        if (data['success']) {
          ...
          if(data['likes'].indexOf(this.currentUserId) === -1) {
            this.likeBtn = "Like";
          } else {
            this.likeBtn = "Unlike";
          }
          console.log(data['likes'].indexOf(this.currentUserId))
